...
```bash
# Setup Docker prerequisites on the DGX
dgx run dmr setup

# Install/upgrade the runner
dgx run dmr install

# Pull models and run prompts
dgx run dmr pull ai/smollm2:360M-Q4_K_M
dgx run dmr run ai/smollm2:360M-Q4_K_M "Write a haiku about GPUs"

# Check health/logs / update
dgx run dmr status
dgx run dmr logs --tail 100
dgx run dmr update
```

Need a custom command? Use the remote helpers:

```bash
# Execute docker model commands directly
dgx exec "docker model run ai/smollm2:360M-Q4_K_M 'Write a haiku about GPUs'"

# Forward the DMR API back home
...
#### Hugging Face / W&B / Codex tokens

```bash
dgx env hf-token                     # prompts for HF token
dgx env wandb --value xxxxx           # set WANDB API key
dgx codex set-api-key --value sk-...  # store CODEX_API_KEY
dgx codex import-config               # sync local ~/.codex to DGX
```
